...
import { searchStocksWithWatchlist } from "@/lib/actions/watchlist.action"; // ✅ CHANGED
...
export default function SearchCommand({ renderAs = 'button', label = 'Add stock', initialStocks }: SearchCommandProps) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const results = await searchStocksWithWatchlist( // ✅ CHANGED
  ...
  return (
    <>
      ...
      <CommandDialog
        ...
        className="z-50 bg-gray-800! lg:min-w-200 border-gray-600 fixed top-10 left-1/2 -translate-x-1/2 translate-y-10"
      >
        <div className="bg-gray-800! border-b border-gray-600 relative">
          <CommandInput
            ...
            className="bg-gray-800! border-0 text-gray-400 placeholder:text-gray-500 focus:ring-0 text-base h-14 pr-10"
          />
          {loading && (
        ...
        <CommandList className="bg-gray-800! max-h-100">
          {loading ? (
            <CommandEmpty className="py-6 bg-transparent! text-center text-gray-500">
              Loading stocks...
            </CommandEmpty>
            ...
            <ul>
              ...
              {displayStocks.map((stock) => (
                <li key={stock.symbol} className="rounded-none my-3 px-1 w-full">
                  <Link
